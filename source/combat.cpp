		...
		if((attackerPlayer = attacker->getPlayer()) || (attackerPlayer = attacker->getPlayerMaster()))
		{
			...
			if(target->isPlayerSummon())
			{
				...
				if(g_game.getWorldType(attackerPlayer, target->getPlayerMaster()) == WORLDTYPE_OPTIONAL &&
					!Combat::isInPvpZone(attacker, target) && !attackerPlayer->isEnemy(target->getPlayerMaster()))
					return RET_YOUMAYNOTATTACKTHISCREATURE;
			}
		}
	}
	/*else if(target->getNpc() && !target->isAttackable())  //fix npc shoot
		return RET_YOUMAYNOTATTACKTHISCREATURE;*/
	...
	if(params.itemId)
	{
		...
		if(player)
		{
			...
			player->addInFightTicks(pzLock);
             ...
             if(Item* item = Item::CreateItem(itemId)) //agregado editado por mi //codigo nuevo  field not show casters field display casters
        { 
            if(caster){
                item->setOwner(caster->getID());
                item->setSpecialDescription("Casted by " + caster->getName());
            }//agregado editado por mi

            if(g_game.internalAddItem(caster, tile, item) == RET_NOERROR)
                g_game.startDecay(item);
            else
                delete item;
} //codigo nuevo  field not show casters field display casters
		/*if(Item* item = Item::CreateItem(itemId))  //codigo antiguo  field not show casters
		{
			if(caster)
				item->setOwner(caster->getID());
			...
			if(g_game.internalAddItem(caster, tile, item) == RET_NOERROR)
				...
				delete item;
		}*/  //codigo antiguo  field not show casters
	}
...
void Combat::addDistanceEffect(Creature* caster, const Position& fromPos, const Position& toPos, ShootEffect_t effect)
{
	if (effect == SHOOT_EFFECT_WEAPONTYPE)
	{
		...
		{
		case WEAPON_AXE:  //agregado editado por mi
